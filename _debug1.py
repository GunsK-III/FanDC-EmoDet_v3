...
#                  "neutral": "中性的", "angry": "愤怒的", "fear": "开心的"}
...
#         [sg.Text("", font=("微软雅黑", 11, "bold"), key="game_text")],
#         # [sg.Column([[sg.Button("捕获(s)")]], justification='left', expand_x=True),
...
#     ]
#     window = sg.Window("在摄像头中识别表情", layout, size=(430, 640), resizable=True)
#     while True:
#         event, values = window.read()
#         if event == "捕获":
#             sg.popup("先打开摄像头！", title="错误")
#         if event == "帮助":
#             help_cap()
#         if event == "打开摄像头":
...
#                 ret, frame = cap.read()
#                 if not ret:
#                     sg.popup("摄像头打开失败！", title="错误")
#                     break
#                 cv2.imshow("Emotion Capture", frame)
#                 key = cv2.waitKey(1) & 0xFF
#                 if key == ord('s') or event == "捕获":
#                     res = emo_model(frame)
#                     resize_frame = cv2.resize(frame, (380, 380))
#                     img_bytes = cv2.imencode(".png", resize_frame)[1].tobytes()
#                     window["image_preview"].update(data=img_bytes)
#                     main_emo = res[0]
#                     conf = res[1]
#                     window["res_text"].update(f"此时你的表情有{conf}的概率是{emo_trans[main_emo]}")
...
#                         window["game_text"].update(game_text)
#                     cv2.imshow("Look at yourself, you are so handsome.", frame)
#                 if key == ord('q'):
#                     break
#             cap.release()
...
#     rand_emo = random.choice(emo_list)
#     # rand_emo_cn = emo_trans[rand_emo]
#     layout = [[sg.Text("打开摄像头捕获表情.")],
#               [sg.Text(f"请在摄像头前做出和 {emo_trans[rand_emo]} 相反的表情。")],
#               [sg.Button("打开摄像头")],
#               [sg.Column([[sg.Button("捕获")]], justification='left', expand_x=True),
#                sg.Column([[sg.Button("帮助")]], justification='center', expand_x=True),
#                sg.Column([[sg.Button("退出")]], justification='right', expand_x=True)],
#               [sg.Text("", key="res_text")],
#               [sg.Text("", key="game_text")]]
#     window = sg.Window("在摄像头中识别表情", layout, size=(350, 200), resizable=True)
#     while True:
#         event, values = window.read()
#         if event == "捕获":
#             sg.popup("先打开摄像头！", title="错误")
...
#         #         main_emo = res[0]
#         #         conf = res[1]
#         #         window["res_text"].update(f"此时你的表情有{conf}的概率是{main_emo}")
#         #         if main_emo != rand_emo:
#         #             game_text = f"你赢了！因为{main_emo}和{rand_emo}是相反的表情。"
#         #             window["game_text"].update(game_text)
#         #         else:
#         #             game_text = f"你失败了！因为画面中人物表情和{rand_emo}并不相反。"
#         #             window["game_text"].update(game_text)
#         #         cv2.imshow("Emotion Capture.", frame)
#         #         if cv2.waitKey(1) & 0xFF == ord('s') or event == "捕获":
#         #             cv2.imshow("Look at yourself,you are so handsome.", frame)
#         #         if cv2.waitKey(1) & 0xFF == ord('q'):
#         #             break
#         #     else:
#         #         sg.popup("摄像头打开失败！", title="错误")
#         """上面的代码中嵌套乱了，会出现一些BUG，我在下面的代码中重新写了一次。下面的应该是正确的。"""
#         if event == "打开摄像头":
#             cap = cv2.VideoCapture(0)
#             while cap.isOpened():
#                 ret, frame = cap.read()
...
#         [sg.Text(f"请在摄像头前做出和 {emo_trans[rand_emo]} 相反的表情。", font=("微软雅黑", 11, "bold"))],
#         [sg.Button("打开摄像头", expand_x=True)],
#         [sg.Frame("图片预览", [[sg.Image(key="image_preview", size=(380, 285))]])],
...
#         [sg.Button("打开摄像头识别", size=(15, 1), button_color=('white', '#0078D7'))],
#         [sg.Button("打开本地图片识别", size=(15, 1), button_color=('white', '#0078D7'))],
#         [sg.Button("打开本地视频识别", size=(15, 1), button_color=('white', '#0078D7'))],
#         [sg.HorizontalSeparator()],
...
# import pydot
# print(pydot.find_graphviz())  # 应该返回 dot 可执行文件的路径
